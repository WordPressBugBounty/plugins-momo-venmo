...
if ( is_checkout() ) {
    wp_register_style( 'wc_venmo_checkout', esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/css/checkout.css') );
    wp_enqueue_style( 'wc_venmo_checkout' );
    $copy_js = 'copy.js';
    if ( !wp_script_is( $copy_js, 'enqueued' ) ) {
        wp_register_script(
            $copy_js,
            esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/' . $copy_js),
            array('jquery', 'wc_venmo_qrcode'),
            null,
            true
        );
        wp_enqueue_script( $copy_js );
        // wp_enqueue_script( 'wc_venmo_copy', esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/copy.js') );
    }
    $qrcode_styling = 'qr-code-styling.min.js';
    if ( !wp_script_is( $qrcode_styling, 'enqueued' ) ) {
        wp_register_script( $qrcode_styling, esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/' . $qrcode_styling) );
        wp_enqueue_script( $qrcode_styling );
        // wp_enqueue_script( 'wc_venmo_qrcode_styling', esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/qr-code-styling.min.js') );
    }
    $qrcode_generator = 'qr-code-generator.js';
    if ( !wp_script_is( $qrcode_generator, 'enqueued' ) ) {
        wp_register_script( $qrcode_generator, esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/' . $qrcode_generator) );
        wp_enqueue_script( $qrcode_generator );
        // wp_enqueue_script( 'wc_venmo_qrcode_generator', esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/qr-code-generator.js') );
    }
    wp_enqueue_script(
        'wc_venmo_qrcode',
        esc_attr(WCVENMO_PLUGIN_DIR_URL . 'assets/js/qrcode.js'),
    ...
    $venmo_note = ( !empty( $this->venmo_note ) ? sprintf( esc_html__( '%s', 'momo-venmo' ), $this->venmo_note ) : sprintf( esc_html__( 'Test Order from %s', 'momo-venmo' ), $domain ) );
